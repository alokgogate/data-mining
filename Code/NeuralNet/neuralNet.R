...
# -----------------------------------------------------------------
#           CREATE REG AND RF MODELS FOR COMPARISON
# -----------------------------------------------------------------
# logistic regression
...
# won't work for 100k unless missing rows found earlier
...
for(i in 1:k){
  ...
  nn <- neuralnet(form,data=train_data,hidden=2,linear.output=F)
  
  nn.predictions <- compute(nn, test_data[, !names(test_data) %in% "churn"])
  nn.predictions <- nn.predictions$net.result*(max(churn_1k$churn)-min(churn_1k$churn))+min(churn_1k$churn)
  
  test.actual <- (test.cv$churn)*(max(churn_1k$churn)-min(churn_1k$churn))+min(churn_1k$churn)
  
  nn.cv[i] <- sum((test.actual - nn.predictions)^2)/nrow(test.cv)
